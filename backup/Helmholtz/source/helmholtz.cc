...
void helmholtz::SetExactSolution(){
    for(int i = 0;i<=N;i++){
        for (int j=0;j<=N;j++)
            uexact[i][j] = sin(10.0*M_PI*x[i])*cos(10.0*M_PI*y[j]);
    }
...
void helmholtz::SetRightHandSide(){
    for (int i = 1;i<N;i++){
        for (int j = 1;j<N;j++){
            b[i][j]  = -200.0*M_PI*M_PI*sin(10.0*M_PI*x[i])*cos(10.0*M_PI*y[j]);
            b[i][j] += 1000.0*( sin(10.0*M_PI*x[i])*cos(10.0*M_PI*y[j]) ) ;
            b[i][j] *= h*h; 
        }
    }

    for (int i = 1;i<N;i++){
        b[i][1]   -= sin(10.0*M_PI*x[i]);
        b[i][N-1] -= sin(10.0*M_PI*x[i]);
    }
...
void helmholtz::ComputeU(){
    ...
    for (int i = 1;i<N;i++){
        u[i][0]   = sin(10.0*M_PI*x[i]);
        u[i][N] = sin(10.0*M_PI*x[i]);
    }
