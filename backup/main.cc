...
void set_b(double** u,double dx,double dy,int Nx,int Ny){
     for (int i=0;i<=Nx;i++){
         for (int j=0;j<=Ny;j++)
            u[j][i] = (1000.0 -200.0*pow(M_PI,2))*sin(10.0*M_PI*i*dx)*cos(10.0*M_PI*j*dy);
     }
}
...
//eigenvalue:
double eigen_value(int p,int q,int nx,int ny,double hx,double hy){
    double value = 0.0;
    value = -( (4.0/hx*hx)*(pow(sin(M_PI*p/2*nx),2))    ) - ( (4.0/hy*hy)*(pow(sin(M_PI*q/2*ny),2)) );
    return value;   
}
